...
// https://github.com/AleksK1NG/Go-CQRS-EventSourcing-Microservice/blob/5d78a3f886163d0d4e388c8f73a6f885aff458be/pkg/es/aggregate.go
const (
	changesEventsCap = 10
	startVersion     = 0
)
...
type AggregateRoot interface {
	...
	GetTenantId() string
	SetTenantId(tenantId string) *AggregateBase
	GetChanges() []any
	ClearChanges()
	GetVersion() uint64
	ToSnapshot()
...
func (a *AggregateBase) GetTenantId() string {
	return a.TenantId
}

func (a *AggregateBase) SetTenantId(tenantId string) *AggregateBase {
	a.TenantId = tenantId
	return a
}
